...
import logging
import os
from botocore.exceptions import ClientError, WaiterConfigError
...
def create_stack(stack_name, template_body):
    ...
    cloudformation_client.get_waiter('stack_exists').wait(
        StackName=stack_name
    )
...
def update_stack(stack_name, template_body):
    try:
        ...
        if 'No updates are to be performed' in str(e):
            logging.info(f'Stack {stack_name} alread updated')
            return e
